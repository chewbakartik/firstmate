...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` before calling the forge's own CLI.
The helper requires a full canonical URL (see [docs/forge-merge-watch.md](forge-merge-watch.md)) and rejects malformed URLs or repo/login override flags before recording merge state.
...
A `https://<host>/<path>/-/merge_requests/<n>` URL invokes `glab mr merge <n> -R https://<host>/<path>`, so the instance comes from the URL, and adds no merge-method flag because the project's own merge method applies.
...
A `https://<host>/<owner>/<repo>/pulls/<n>` URL invokes `tea pulls merge <n> --repo <owner>/<repo> --login <name>` for Gitea/Forgejo, resolving the tea login whose own endpoint matches the parsed host's endpoint (see [docs/forge-merge-watch.md](forge-merge-watch.md) for the default-port normalization), and defaults to `--style squash` while preserving explicit merge-method flags.
After any forge command returns, the script confirms the PR or MR actually landed, and only a confirmed landing records a landed outcome; a queued or unconfirmed request records none and leaves its poll armed.
